...
export class FuseVerticalNavigationCollapsableItemComponent implements OnInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        this._navigationService.currentActiveRoute.pipe(takeUntil(this._unsubscribeAll)).subscribe((val) => {
            if (this._hasActiveChild(this.item, this._router.url, val)) {
                this.expand();
            }
        })
    }
     ...
     * @private
     */
    private _hasActiveChild(item: FuseNavigationItem, currentUrl: string, activeRoute: any = {}): boolean
    {
        ...
        for ( const child of children )
        {
            ...
            if( child.title === activeRoute.title) {
                return true;
            }
        }
